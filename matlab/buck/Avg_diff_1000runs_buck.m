...
close all;
clear all;
file_path = 'D:\SIMRAN_GUPTA_INTERN\CSV\buck\reduced2031_mcp_1000runs.csv';  % faultfree MonteCarlo data
...
noOfData=size(faultfree,1);%no of rows
NMC=1000;
% vpp=9.07834;

order=7;
Vin1=d{1002};   % first input
Vin2=d{1003};   % second input
for n = 1:1000		%except 1st row 1st row is time
	y=d{n+1};                           % output of each run
    % Vin1=d{2502};   % first input
    % Vin2=d{2503};   % second input
    p= polyfitn([d{1002},d{1003}],y,order);  % doing polyfitn to get the polynomial regression
